...
void task_Principal(void)
{
	...
	// Tabuleiro
	/*TASK taskTabuleiro;
	char *pstackTabuleiro;
	
	PRIORITY taskpri = 5;
	TASKPROP taskprop;
	ksize_t stacksize = 1024;
	
	KS_OpenTask((char *)0, &taskTabuleiro);
	pstackTabuleiro = (char *)KS_AllocBlk(STKPART);
	
	taskprop.priority = taskpri;
	taskprop.stackbase = pstackTabuleiro;
	taskprop.stacksize = stacksize;
	taskprop.taskentry = task_Tabuleiro;
		
	KS_DefTaskProp (taskTabuleiro, &taskprop);
	KS_ExecuteTask (taskTabuleiro);
	*/
	TASK taskBola[5];
	char *pstackBola[5];
	...
	for(int i=0;i<qtdBola;i++) 
	{
		...
		taskprop.stacksize = stacksize;
		taskprop.taskentry = task_Mov_Bola;
